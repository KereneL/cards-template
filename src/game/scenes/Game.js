...
import { CardZone } from '../game-objects/zones/base-zone';
...
import { onDragStart, onDrag, onDragEnter, onDragLeave, onDragOver, onDrop, onDragEnd } from '../game-objects/zones/behaviors/drag-behaviors';
 export class Game extends Phaser.Scene {
    ...
    create() {
        ...
        this.enemy = new CardZone(this, 512, 250, 600, 150, (card, zone) => {
            const cardName = PlayingCardComponent.getComp(card).name
            console.log(`Dropped ${cardName || 'card'} on enemy hand`);
        });

        this.tableu = new CardZone(this, 512, 425, 600, 150, (card, zone) => {
            const cardName = PlayingCardComponent.getComp(card).name
            console.log(`Dropped ${cardName || 'card'} on tableu`);
        });

        this.hand = new CardZone(this, 512, 600, 600, 150, (card, zone) => {
            const cardName = PlayingCardComponent.getComp(card).name
